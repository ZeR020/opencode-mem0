...
export function classifyMemory(
  memoryType?: string
): { storeType: "stm" | "ltm"; decayRate: number } {
  ...
  try {
    ...
    for (const shard of allShards) {
      try {
        ...
        const memory = db
          .prepare(
            `SELECT id, store_type, strength, access_count FROM memories WHERE id = ?`
          )
          .get(memoryId) as any;
        ...
        if (strength > threshold && accessCount > 3) {
          db.prepare(
            `UPDATE memories SET store_type = 'ltm', decay_rate = 0.01 WHERE id = ?`
          ).run(memoryId);
    ...
    for (const shard of allShards) {
      try {
        ...
        for (const memory of memories) {
          ...
          if (
            clampedStrength < archiveThreshold &&
  ...
  try {
    ...
    for (const shard of allShards) {
      try {
        const db = connectionManager.getConnection(shard.dbPath);
        const result = db
          .prepare(`SELECT COUNT(*) as count FROM memories_archive`)
          .get() as any;
