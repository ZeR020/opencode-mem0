    ...
    if (CONFIG.opencodeProvider && CONFIG.opencodeModel) {
      ...
      if (isProviderConnected(CONFIG.opencodeProvider)) {
        ...
        const result = await generateStructuredOutput({
          ...
          systemPrompt: "You are a precise contradiction detector. Analyze two statements and answer ONLY YES or NO. Be strict: only answer YES if the statements are logically incompatible.",
          userPrompt: prompt,
          schema,
          temperature: 0.0,
        });
...
function findSimilarMemories(
  ...
  if (results.length === 0) {
    ...
    results = rows.map((r) => {
      const rWords = r.content.toLowerCase().split(/\s+/).filter((w: string) => w.length > 3);
...
export async function resolveConflict(
  ...
  try {
    ...
    const shards = [...shardManager.getAllShards("user", ""), ...shardManager.getAllShards("project", hash)];
    ...
    for (const shard of shards) {
      ...
      switch (strategy) {
        case "keep_newer": {
          ...
          const mem1 = db.prepare("SELECT id, created_at FROM memories WHERE id = ?").get(conflict.memoryId1) as any;
          const mem2 = db.prepare("SELECT id, created_at FROM memories WHERE id = ?").get(conflict.memoryId2) as any;
        ...
        case "merge": {
          ...
          const mem1 = db.prepare("SELECT * FROM memories WHERE id = ?").get(conflict.memoryId1) as any;
          if (!mem1) {
          ...
          `
            ...
            now,
            now,
            JSON.stringify({ mergedFrom: [conflict.memoryId1, conflict.memoryId2], originalType: mem1.type }),
            mem1.display_name,
            mem1.user_name,
            mem1.user_email,
            mem1.project_path,
