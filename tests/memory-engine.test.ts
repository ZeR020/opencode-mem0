...
function makeDb(path: string) {
  const rows = path.includes("shard-a")
    ? [{ id: "a", content: "A", created_at: 2, container_tag: "tag-a", is_deprecated: 0, store_type: "stm", strength: 0.5, access_count: 0 }]
    : path.includes("shard-b")
      ? [{ id: "b", content: "B", created_at: 1, container_tag: "tag-b", is_deprecated: 0, store_type: "stm", strength: 0.5, access_count: 0 }]
      : [{ id: "c", content: "C", created_at: 3, container_tag: "current", is_deprecated: 0, store_type: "stm", strength: 0.5, access_count: 0 }];
          ...
          if (sql.includes("transcripts")) {
            ...
            if (sql.includes("MATCH")) {
              ...
              return transcriptRows.filter((r) =>
                r.messages.toLowerCase().includes(query)
              );
            }
            const tag = args[0];
...
mock.module("../src/config.js", () => ({
  CONFIG: {
    ...
    memoryScoring: { enabled: true, recalculationIntervalMinutes: 60, recencyHalfLifeDays: 7, utilityHalfLifeDays: 3 },
    memoryLifecycle: { stmDecayDays: 7, ltmDecayDays: 90, promotionThreshold: 0.7, archiveThreshold: 0.2, archiveAfterDays: 30, checkIntervalMinutes: 60 },
    retrieval: { maxResults: 20, diversityThreshold: 0.9, contextBoost: 1.5 },
  },
}));

// Import the modules under test (must be after mocks)
const { calculateRecency, calculateFrequency, calculateImportance, calculateUtility, calculateNovelty, calculateConfidence, calculateInterference, computeStrength, calculateAllScores, recordAccess } = await import("../src/services/memory-scoring.js");
const { classifyMemory, promoteToLTM } = await import("../src/services/memory-lifecycle.js");
const { calculateContextBoost, calculateDiversityPenalty, contextTracker } = await import("../src/services/retrieval-context.js");
...
describe("Memory Engine Integration", () => {
  ...
  describe("transcript storage", () => {
    ...
    it("returns recent transcripts ordered by time", () => {
      ...
      db.prepare("UPDATE transcripts SET created_at = ? WHERE session_id = ?").run(now - 2000, "sess-1");
      mgr.saveTranscript("sess-2", "/project/b", [{ content: "second" }]);
      db.prepare("UPDATE transcripts SET created_at = ? WHERE session_id = ?").run(now - 1000, "sess-2");
    ...
    it("deletes old transcripts", () => {
      ...
      const db = (mgr as any).getDb();
      db.prepare("UPDATE transcripts SET created_at = ? WHERE session_id = ?").run(oldTime, "sess-old");
  ...
  describe("memory scoring", () => {
    ...
    it("calculates importance with technical keywords", () => {
      ...
      const high = calculateImportance("Implement async function with TypeScript interface and database schema migration");
    ...
    it("calculates novelty as inverse of similarity", () => {
      const novel = calculateNovelty("Completely unique topic about quantum computing", ["Common programming patterns"]);
      const duplicate = calculateNovelty("Common programming patterns", ["Common programming patterns"]);
    ...
    it("calculates interference for contradictory memories", () => {
      const penalty = calculateInterference("Added feature X", ["Removed feature X", "Feature X is disabled"]);
    ...
    it("computes overall strength from components", () => {
      ...
      const weakScores = { ...scores, recency: 0, frequency: 0, importance: 0, utility: 0, novelty: 0, confidence: 0, interference: 1.0 };
  ...
  describe("memory conflicts", () => {
    it("detects conflicts via heuristic fallback", async () => {
      ...
      db.prepare = (sql: string) => {
        if (sql.includes("sqlite_master")) {
          return { get: () => ({ name: "memories_fts" }), all: () => [{ name: "memories_fts" }], run: () => {} };
        }
        ...
        if (sql.includes("memory_conflicts")) {
          return {
            all: () => [],
            get: () => null,
            run: () => { conflictSaved = true; return { changes: 1 }; },
          };
        }
        return {
          all: () => [],
      ...
      const conflicts = await detectConflicts("new-1", "Use TypeScript for the project, not JavaScript", "mem_project_abc");
    ...
    it("resolves conflict with keep_newer strategy", async () => {
      ...
      db.prepare = (sql: string) => {
        ...
        if (sql.includes("UPDATE memories SET is_deprecated = 1")) {
          return {
            run: (id: string) => { deprecatedId = id; return { changes: 1 }; },
            get: () => null,
            all: () => [],
          };
        }
