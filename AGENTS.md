...
- Changelog entries describe changes only — never credit people. No `@mentions`, no `Contributors:` lists in `docs/CHANGELOG.md`. Credit comes from git itself: the release workflow extracts contributors from commit authorship between the previous and current tag and appends them to the GitHub release notes under a **Contributors** heading. That is the only place usernames belong.
